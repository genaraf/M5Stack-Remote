...
void callibration() {
  ...
  Disbuff.setCursor(STATUS_HEIGHT, 7);
  ...
  while(cnt++ <= CALL_NUMB) {
    ...
    displayData();
    delay(1000);       
    Disbuff.fillRect(0, 0, 10, STATUS_HEIGHT, BLACK);
    Disbuff.setTextColor(RED);
...
//  M5.Axp.SetChargeCurrent(CURRENT_100MA);
  ...
  if (( EEPROM.read(0) != 0x56 ) || ( M5.BtnA.read() == 1 ))
  {
    ...
    if (n == 0) {
      Disbuff.setCursor(5, STATUS_HEIGHT);
      ...
      for (int i = 0; i < n; ++i) {
        if ( WiFi.SSID(i).indexOf("M5AP") != -1 )
        {
          ...
          Disbuff.setCursor(5, STATUS_HEIGHT + count * 10 );
...
void displayData() {
  ...
  Disbuff.setCursor(TEXT_LEFT, line += LINE_SPACE);
...
void displayStatus() {
  Disbuff.setTextColor(GREEN);
  Disbuff.fillRect(STATUS_HEIGHT, 0, LCD_WIDTH, STATUS_HEIGHT, Disbuff.color565(50, 50, 50));
  Disbuff.setCursor(RSSI_OFSET, 7);
  if(connected) {
